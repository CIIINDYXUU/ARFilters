...
class CGAnimatorController extends BaseNode {
  ...
  _updateEventListener() {
    this.haveRegisteredListener = this.sys.eventListener.haveRegistered(
  ...
  onDestroy(sys) {
    sys.eventListener.removeListener(sys.script, Amaz.AnimazEventType.ANIM_END, this.currentClip, sys.script);
  }
}
